    ...
    for group in groups:
        ...
        if possible_targets:
            if len(possible_targets) == 1:
                ...
                options = list()
                damage_level = sorted(
                    possible_targets, key=lambda item: item[0], reverse=True
                )[0][0]
...
def start_fight(immunes: List[Group], infections: List[Group]):
    while True:
        immunes = sorted(
            immunes,
        ...
        if all((group.units == 0 for group in immunes)):
            print(sum(group.units for group in infections))
            return
...
def main():
    immunes, infections = read_inputs()
    start_fight(immunes, infections)
