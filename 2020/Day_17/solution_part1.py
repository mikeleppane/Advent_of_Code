        ...
        for row, line in enumerate(f_handle):
            ...
            if line:
                for column, char in enumerate(list(line)):
                    ...
                    cube_states[Coordinate(x=column, y=row, z=0)] = cube_state

    return PocketDimension(cubes=cube_states)


def get_cube_neighbor_indexes(x, y, z):
    indexes = list()
    for cube_index in ADJACENT_INDICES_3D:
        indexes.append(tuple(map(add, (x, y, z), cube_index)))
    return indexes
    ...
    for _ in range(6):
        ...
        for coordinate, state in prev_pocket_dim.cubes.items():
            ...
            if state == State.ACTIVE:
                ...
                for x, y, z in adjacent_indexes:
                    if prev_pocket_dim.cubes.get(Coordinate(x, y, z)) == State.ACTIVE:
                        number_of_active_adjacent_cubes += 1
                    elif Coordinate(x, y, z) not in prev_pocket_dim.cubes:
                        if (
                            sum(
                                (
                                    1
                                    for x_2, y_2, z_2 in get_cube_neighbor_indexes(
                                        x, y, z
                                    )
                                    if prev_pocket_dim.cubes.get(
                                        Coordinate(x_2, y_2, z_2)
                                    )
                                    == State.ACTIVE
                                )
                            )
                            ...
                            pocket_dimension.cubes[Coordinate(x, y, z)] = State.ACTIVE
                ...
                number_of_active_adjacent_cubes = 0
                for x, y, z in adjacent_indexes:
                    if (x, y, z) in prev_pocket_dim.cubes and prev_pocket_dim.cubes[
                        Coordinate(x, y, z)
                    ] == State.ACTIVE:
                        number_of_active_adjacent_cubes += 1
                    elif Coordinate(x, y, z) not in prev_pocket_dim.cubes:
                        adjacent_indexes_2 = get_cube_neighbor_indexes(x, y, z)
                        number_of_active_adjacent_cubes_2 = 0
                        for x_2, y_2, z_2 in adjacent_indexes_2:
                            if (
                                x_2,
                                y_2,
                                z_2,
...
def set_3d_adjacent_indices():
    for index_3d in set(permutations([-1, -1, -1, 1, 1, 1, 0, 0], 3)):
        ADJACENT_INDICES_3D.add(index_3d)


def main():
    set_3d_adjacent_indices()
